...
export default function Business() {
  ...
  const load = async () => {
    ...
    setIncome((payments || []).reduce((s, p) => s + +p.amount, 0))
    setExpenses(exps || [])
    setLoading(false)
  }
    ...
    <div style={{ direction:'rtl' }}>
      ...
      {loading ? (
        ...
        <>
              ...
              {income === 0 && totalExpenses === 0
                ...
                : <>
                  <div style={{ marginBottom:16 }}>
                    <div style={{ display:'flex', justifyContent:'space-between', marginBottom:5 }}>
                      <span style={{ fontSize:13, color:'#10b981', fontWeight:600 }}>💰 הכנסות</span>
                      <span style={{ fontSize:13, fontWeight:700, color:'#10b981' }}>₪{income.toLocaleString()}</span>
                    </div>
                    <div style={{ height:10, background:'#f1f5f9', borderRadius:10 }}>
                      <div style={{ height:'100%', width:`${income+totalExpenses > 0 ? (income/(income+totalExpenses)*100) : 0}%`, background:'#10b981', borderRadius:10, transition:'width 0.5s ease' }} />
                    </div>
                  </div>
                  <div style={{ marginBottom:20 }}>
                    <div style={{ display:'flex', justifyContent:'space-between', marginBottom:5 }}>
                      <span style={{ fontSize:13, color:'#ef4444', fontWeight:600 }}>💸 הוצאות</span>
                      <span style={{ fontSize:13, fontWeight:700, color:'#ef4444' }}>₪{totalExpenses.toLocaleString()}</span>
                    </div>
                    <div style={{ height:10, background:'#f1f5f9', borderRadius:10 }}>
                      <div style={{ height:'100%', width:`${income+totalExpenses > 0 ? (totalExpenses/(income+totalExpenses)*100) : 0}%`, background:'#ef4444', borderRadius:10, transition:'width 0.5s ease' }} />
                    </div>
                  </div>
                  <div style={{ background: balance>=0 ? '#ecfdf5' : '#fef2f2', borderRadius:12, padding:'12px 16px', textAlign:'center' }}>
          ...
          {/* רשימת הוצאות */}
          <div style={{ background:'#fff', borderRadius:16, border:'1px solid #f1f5f9', boxShadow:'0 1px 4px rgba(0,0,0,0.04)', marginTop:20, overflow:'hidden' }}>
          ...
          <div style={{ background:'#fff', borderRadius:20, padding:32, width:420, direction:'rtl', boxShadow:'0 24px 60px rgba(0,0,0,0.15)', animation:'fadeUp 0.25s ease' }}>
            ...
            <div style={{ marginBottom:14 }}>
              ...
              <select style={inp} value={form.category} onChange={e => setForm(p => ({...p, category:e.target.value}))}>
                {Object.entries(EXP_CAT).map(([k,v]) => (
